...
odświeżania ekranu (typowo ok.~16,7~ms przy częstotliwości 60~Hz)\footnote{\raggedright \url{https://blurbusters.com/blur-busters-law-amazing-journey-to-future-1000hz-displays-with-blurfree-sample-and-hold/} (21.09.2026)},
...
dodatkowo: cykliczne działanie mechanizmu czysczenia pamięci
(\textit{garbage collector})\footnote{\url{https://developer.chrome.com/docs/devtools/memory-problems} (21.09.2026)}, synchronizacja wertykalna obrazu (V-Sync)
...
(UE) 2016/679 (RODO) \footnote{\url{https://uodo.gov.pl/404} (21.09.2026)} szczególną kategorię danych osobowych, ujętą
...
trzecich z~zewnątrz infrastruktury, lecz nie zabezpieczają danych przed
...
Bezpieczeństwo architektury aplikacji opartej o~szablon Electrona wymagało ścisłego oddzielenia logiki biznesowej renderera od możliwości niskopoziomowych procesu głównego. W~tym celu w~systemie całkowicie wyłączono integrację z~Node.js po stronie renderera (\texttt{nodeIntegration: false}) oraz włączono pełną izolację kontekstu (\texttt{contextIsolation: true}).\footnote{\url{https://www.electronjs.org/docs/latest/tutorial/security} (21.09.2026)}

Jedynym pomostem umożliwiającym komunikację między procesami jest skrypt preload (\texttt{preload.js}), który przy użyciu modułu \texttt{contextBridge}\footnote{\url{https://www.electronjs.org/docs/latest/api/context-bridge} (21.09.2026)} udostępnia w~globalnym obiekcie \texttt{window} bezpieczne, zminimalizowane API o~nazwie \texttt{electronAPI}. Implementacja ta zapobiega wstrzykiwaniu dowolnego kodu Node.js przez skrypty testowe i~renderer. Komunikacja opiera się na ściśle zdefiniowanych kanałach IPC:
\begin{itemize}
...
Kiedy należy korzystać z HPM - jeśli w badaniu kluczowy jest czas reakcji to korzystamy z HPM, natomiast w przypadku badania innych funkcji poznawczych (pamięć, koncentracja, myślenie) HPM staje się zbędny.
...
\begin{itemize}
	...
	\item \textbf{Eksport do~formatów klinicznych:} Obsługa formatów SPSS (.sav) i REDCap z~pełnymi metadanymi ułatwiłaby integrację z~istniejącymi pipeline'ami badawczymi.
	...
	\item \textbf{Zakup certyfikatów ~Code Signing} Podpisanie programu usprawniło by proces instalacji oraz zwiększyło by wiarygodność oprogramowania.
\end{itemize}
