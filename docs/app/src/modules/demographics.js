...
import {
  ...
  registerFillFormCallback,
  saveCurrentAsParticipant,
  renderParticipantsList,
} from "./participants.js";
import { getResearcherUid } from "./auth.js";

/**
 * Zwraca klucze localStorage specyficzne dla aktualnego użytkownika.
 * Zapobiega przeciekaniu danych demograficznych między kontami.
 */
function getDemoStorageKeys() {
  const uid = getResearcherUid().replace("::", "__");
  return {
    templateId: `activeTemplateId_${uid}`,
    demographics: `activeDemographics_${uid}`,
  };
}
...
export async function initDemographics() {
  ...
  const savedTemplateId = localStorage.getItem(getDemoStorageKeys().templateId);
  if (savedTemplateId) {
  ...
  elements.demoTemplateSelect.addEventListener("change", async (e) => {
    ...
    localStorage.setItem(getDemoStorageKeys().templateId, val);
  ...
  if (btnSaveToRegistry) {
    btnSaveToRegistry.addEventListener("click", async () => {
      if (!activeDemographics) {
        await Dialog.alert(
          'Najpierw zapisz dane sesji (kliknij "Zapisz Dane Sesji").',
          "warning",
        );
        return;
      }
...
export async function refreshTemplatesDropdown() {
  ...
  elements.demoTemplateSelect.innerHTML =
    '<option value="">-- Domyślna (Brak) --</option>';
  templates.forEach((t) => {
...
async function renderDynamicForm(templateId) {
  ...
  if (!templateId) {
    container.innerHTML =
      '<p style="color:#888;">Wybierz szablon, aby wypełnić metryczkę.</p>';
    return;
  }
  ...
  if (!template) {
    container.innerHTML =
      '<p style="color:red;">Błąd: Szablon nie istnieje.</p>';
    return;
  }
...
export function loadSavedDemographics() {
  const saved = localStorage.getItem(getDemoStorageKeys().demographics);
  if (saved) {
    try {
      ...
      if (
        activeDemographics &&
        activeDemographics.templateId === currentTemplateId
      ) {
...
export async function saveDemographicsFromForm(forceOverwrite = false) {
  ...
  const hasInputs = Array.from(inputs).some((i) => {
    ...
    if (i.dataset.type === "checkbox" && i.dataset.checkboxGroup !== "true")
      return false;
    return true;
  });
  ...
  localStorage.setItem(
    ...
    JSON.stringify(activeDemographics),
  );
...
export async function clearDemographics() {
  ...
  const confirmed = await Dialog.confirm(
    "Czy na pewno chcesz usunąć zapisane dane uczestnika?",
  );
  if (confirmed) {
    activeDemographics = null;
    _loadedParticipantDisplayName = null;
    localStorage.removeItem(getDemoStorageKeys().demographics);
